...
class evolutionnaryAlgorithm():
    ...
    def computeFitnesses(self, population):
        ...
        fitnesses = []
        for p in population:
            reward = tools.getFitness_part3(p)
    ...
    def selectionESet(self, population, fitnesses):
        
        candidateWords = []
        for i in range(len(population)):
            if tools.respectsAllConstraints(population[i]):
                candidateWords.append([population[i], fitnesses[i]])
